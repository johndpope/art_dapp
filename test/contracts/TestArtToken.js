...
contract('ArtToken', function(accounts) {
    ...
    it("Test if the contract has the correct name and token", function() {
      return ArtToken.deployed().then(function(instance) {
        return instance.name();
    }).then(function(name) {
        assert.equal(name, 'ArtToken', "ArtToken is not the name of the ");
      });
    });
  ...
  //     assert.equal(balance.valueOf(), 0, "1776 wasn't in the first account");
  //   });
  // });

  // it("should mint a token", function() {
  //     return ArtToken.new(200).then(function(instance) {
  //       return instance.mint.call(accounts[5], 0);
...
//     this.token = await ArtToken.new(name, symbol, { from: creator });
//   });
//   //
